...
class CartCounter extends StatefulWidget {
  ...
  @override
  _CartCounterState createState() => _CartCounterState(this.product);
}

void CartCounterUpdate(var qty, var c_id, var context) async {
  final response = await http.post(
      Uri.parse("https://shopera-app01.000webhostapp.com/uploadCartCounter.php"),
      body: {"c_id": c_id, "qty": qty});
  ...
  //         cart_products[k]["qty"] = cart_list[i]["qty"];
  //         k++;
          ...
          press: () {
            if (numOfItems > 1) {
              setState(() {
                numOfItems--;
                product["qty"]--;
                total = total - int.parse(product["sp"]);
                print(total);
                CartCounterUpdate(product["qty"].toString(),
                    product["c_id"].toString(), context);
                Navigator.pushReplacement(
                  context,
                  MaterialPageRoute(builder: (context) => CartNavigator()),
                );
